...
var local_lobby_name: String = ""  # This is room name or code. Not the actually code needed on GDSync side
...
func _ready() -> void:
	...
	set_process(false)
	logger.log_info("LocalServerWebPatch initialized for web platform.")
	my_current_client = Client.new()
	my_current_client.valid = true
	my_current_client.client_id = multiplayer.get_unique_id()

	# Register JS callbacks for browser console testing (web only)
	if OS.has_feature("web"):
		_register_js_test_callbacks()
...
func create_local_lobby(
	...
	if session is PRSession:
		...
		await _update_room_metadata(
			session.code, lobby_name, public, player_limit
		)
		rtc_session = session
		is_host = true
		...
		host_client.peer_id = my_current_client.client_id
		host_client.client_id = my_current_client.client_id
		host_client.valid = true
		peer_client_table[my_current_client.client_id] = host_client
		lobby_client_table[my_current_client.client_id] = host_client

		set_process(true)
		_register_rtc_callbacks()  # Enable browser console testing
		logger.log_info(
			(
				"Web lobby created. Code: "
				+ current_room_code
				+ ", Host ID: "
				+ str(my_current_client.client_id)
			)
		)
		...
		logger.log_error("Failed to create web lobby: " + str(session))
		GDSync.lobby_creation_failed.emit.call_deferred(
...
func join_lobby(lobby_name: String, password: String) -> void:
	...
	if session is PRSession:
		...
		logger.log_info(
			(
				"After setting peer, multiplayer unique_id=%s"
		...
		logger.log_info(
			(
				"Joined web lobby (signaling complete). My ID: "
				+ str(my_current_client.client_id)
				+ ", Host ID: 1"
			)
		)
		...
		if 1 in already_connected_peers and _pending_join_lobby_name != "":
			logger.log_info(
				"Host already in peers list! Manually triggering join flow..."
			)
			...
			(
				logger
				. log_info(
					"Host peer exists via has_peer(1). Checking data channel state..."
				)
			)
		...
		logger.log_error("Failed to join web lobby: " + str(session))
...
func _process_host_outgoing_requests(requests: Array) -> void:
	for req in requests:
		if req is Array and req.size() >= 2:
			var req_type := int(req[0])
			if req_type == int(ENUMS.REQUEST_TYPE.MESSAGE):
				_handle_message_request(req)


func _handle_message_request(req: Array) -> void:
	var msg_type := int(req[1])

	match msg_type:
		int(ENUMS.MESSAGE_TYPE.LOBBY_JOINED):
			# req looks like: [REQUEST_TYPE.MESSAGE, LOBBY_JOINED, lobby_id]
			var lobby_id := str(req[2])
			current_room_code = lobby_id
			connection_controller.in_local_lobby = true
			connection_controller.status = (
				ENUMS.CONNECTION_STATUS.LOCAL_CONNECTION
			)
			GDSync.lobby_joined.emit.call_deferred(lobby_id)

		int(ENUMS.MESSAGE_TYPE.LOBBY_JOIN_FAILED):
			# [MESSAGE, LOBBY_JOIN_FAILED, lobby_id, error_code]
			var lobby_id := str(req[2])
			var err := int(req[3])
			GDSync.lobby_join_failed.emit.call_deferred(lobby_id, err)

		int(ENUMS.MESSAGE_TYPE.HOST_CHANGED):
			# [MESSAGE, HOST_CHANGED, host_id]
			var host_id := int(req[2])
...
func _on_lobbies_request_completed(
	result: int,
	response_code: int,
...
func _on_peer_connected(id: int) -> void:
	logger.log_info(
		"WebRTC peer connected: " + str(id) + ", is_host: " + str(is_host)
	)
	...
	if rtc_session and rtc_session.rtc_peer:
		...
		if peer_info:
			logger.log_info(
				"Peer " + str(id) + " connection details: " + str(peer_info)
			)
			var channels = peer_info.get("channels", [])
			logger.log_info(
				"Peer " + str(id) + " has " + str(channels.size()) + " channels"
			)
	...
	if not is_host and id == 1 and _pending_join_lobby_name != "":
		logger.log_info(
			"Connected to host! Sending GDSync join protocol messages..."
		)
		...
		request_processor.create_join_lobby_request(
			_pending_join_lobby_name, _pending_join_password
		)
		# Clear pending state
...
func _send_client_packets_immediately() -> void:
	...
	logger.log_info(
		(
			"Sending packets immediately - SETUP: "
			+ str(has_setup)
			+ ", SERVER: "
			+ str(has_server)
			+ ", RELIABLE: "
	...
	logger.log_info(
		(
			"Peer state before sending - connection_status: "
			+ str(conn_status)
			+ ", has_peer(1): "
			+ str(has_peer_1)
		)
	...
	if conn_status != MultiplayerPeer.CONNECTION_CONNECTED:
		logger.log_warning(
			"Cannot send packets - not connected! Status: " + str(conn_status)
		)
		return
	...
	peer.set_target_peer(1)  # HARDCODED SEND TO HOST WITH ID 1

	if has_setup:
		var packet_data = request_processor.package_requests(
			ENUMS.PACKET_CHANNEL.SETUP
		)
		logger.log_info(
			(
				"CLIENT sending SETUP packet to host (immediate), size: "
	...
	if has_server:
		...
		logger.log_info(
			(
				"CLIENT sending SERVER packet to host (immediate), size: "
				+ str(packet_data.size())
				+ ", first 20 bytes: "
				+ str(packet_data.slice(0, min(20, packet_data.size())))
			)
		)
		...
		if peer_info_before:
			...
			if channels_before.size() > 0 and channels_before[0]:
				logger.log_info(
					(
						"Channel 0 BEFORE send: buffered="
						+ str(channels_before[0].get_buffered_amount())
					)
				)
		...
		if peer_info:
			logger.log_info(
				(
					"Peer 1 info after send: connected="
					+ str(peer_info.get("connected", "?"))
					+ ", channels="
					+ str(peer_info.get("channels", []).size())
				)
			)
	...
	if has_reliable:
		...
		logger.log_info(
			(
				"CLIENT sending RELIABLE packet to host (immediate), size: "
				+ str(packet_data.size())
			)
		)
		peer.transfer_mode = MultiplayerPeer.TRANSFER_MODE_RELIABLE
	...
	if has_unreliable:
		...
		logger.log_info(
			(
				"CLIENT sending UNRELIABLE packet to host (immediate), size: "
				+ str(packet_data.size())
			)
		)
...
func _trigger_join_flow() -> void:
	...
	logger.log_info(
		"Triggering join flow - sending GDSync join protocol messages..."
	)
	# Send client ID first
	request_processor.send_client_id()
	# Broadcast player data
	...
	# Send join lobby request
	request_processor.create_join_lobby_request(
		local_lobby_name, local_lobby_password
	)
		...
		if peer.has_peer(1):
			...
			if peer_info and peer_info.has("connection"):
				...
				if conn_state == WebRTCPeerConnection.STATE_CONNECTED:
					logger.log_info(
						"Host data channel is ready! Triggering join flow..."
					)
					_waiting_for_host_data_channel = false
		...
		MultiplayerPeer.CONNECTION_CONNECTED:
			...
			if immediate_pkt_count > 0:
				logger.log_info(
					(
						"POLL: "
						+ str(immediate_pkt_count)
						+ " packets available after poll, is_host="
						+ str(is_host)
					)
				)
			...
			if is_host:
				...
				# --- HOST: send queued requests to each client using GDSync packaging ---
				for client_id in lobby_client_table.keys():
					...
					# Send per-channel the same way your client does
					for ch in [
						ENUMS.PACKET_CHANNEL.SERVER,
						ENUMS.PACKET_CHANNEL.RELIABLE,
						ENUMS.PACKET_CHANNEL.UNRELIABLE
				...
				if available > 0:
					logger.log_info(
						"HOST has " + str(available) + " packets available"
					)
				...
				while peer.get_available_packet_count() > 0:
					...
					logger.log_warning(
						(
							"[HOST] Received packet from peer "
							+ str(sender_peer_id)
							+ ", size: "
							+ str(bytes.size())
						)
					...
					if not peer_client_table.has(sender_peer_id):
						logger.log_warning(
							"Unknown sender peer: " + str(sender_peer_id)
						)
						continue
				...
				var has_setup = request_processor.has_packets(
					ENUMS.PACKET_CHANNEL.SETUP
				)
				var has_server = request_processor.has_packets(
					ENUMS.PACKET_CHANNEL.SERVER
				)
				var has_reliable = request_processor.has_packets(
					ENUMS.PACKET_CHANNEL.RELIABLE
				...
				if has_setup:
					...
					peer.transfer_mode = MultiplayerPeer.TRANSFER_MODE_RELIABLE
					peer.put_packet(
						request_processor.package_requests(
							ENUMS.PACKET_CHANNEL.SETUP
						)
					)
				if has_server:
				...
				while peer.get_available_packet_count() > 0:
					...
					logger.log_info(
						(
							"CLIENT received packet from host, size: "
							+ str(bytes.size())
						)
					)
...
func _process_server_request(from: Client, request: Array) -> void:
	...
	logger.log_info(
		(
			...
			+ str(ENUMS.REQUEST_TYPE.keys()[request_type])
			+ " from "
			+ str(from.client_id)
		)
	)
...
func _join_lobby_request(from: Client, request: Array) -> void:
	...
	logger.log_info(
		"Client " + str(from.client_id) + " joined lobby " + local_lobby_name
	)
...
func _update_room_metadata(
	code: String, lobby_name: String, public: bool, player_limit: int
) -> void:
	if signaling_server_url == "":
		return
	...
	if result[1] == 200:
		...
		logger.log_error(
			"Failed to update room metadata, status: " + str(result[1])
		)
...
func _update_player_count_on_server() -> void:
	...
	if result[1] != 200:
		logger.log_error(
			"Failed to update player count, status: " + str(result[1])
		)
...
## ============================================================================
## DEBUG: Bidirectional WebRTC Test Functions
## After connecting, call from browser console:
##   window.rtcTest.ping()       - Send a test ping
##   window.rtcTest.channels()   - Check channel states
##   window.rtcTest.poll()       - Force poll for packets
## ============================================================================


## Register JavaScript callbacks for browser console testing
func _register_js_test_callbacks() -> void:
	var js_code = """
	window.rtcTest = {
		ping: function() {
			if (window._godot_rtc_ping) window._godot_rtc_ping();
			else console.log('Not ready - create/join a lobby first');
		},
		channels: function() {
			if (window._godot_rtc_channels) window._godot_rtc_channels();
			else console.log('Not ready - create/join a lobby first');
		},
		poll: function() {
			if (window._godot_rtc_poll) window._godot_rtc_poll();
			else console.log('Not ready - create/join a lobby first');
		}
	};
	console.log('🎮 WebRTC test functions registered! Use: rtcTest.ping(), rtcTest.channels(), rtcTest.poll()');
	"""
	JavaScriptBridge.eval(js_code)


## Called when RTC session is ready - register the actual callbacks
func _register_rtc_callbacks() -> void:
	if not OS.has_feature("web"):
		return
	# Create callable references that JS can invoke
	var ping_cb = JavaScriptBridge.create_callback(_js_test_ping)
	var channels_cb = JavaScriptBridge.create_callback(_js_test_channels)
	var poll_cb = JavaScriptBridge.create_callback(_js_test_poll)

	# Store references to prevent GC
	_js_callbacks = [ping_cb, channels_cb, poll_cb]

	# Assign to window
	JavaScriptBridge.get_interface("window").set("_godot_rtc_ping", ping_cb)
	JavaScriptBridge.get_interface("window").set(
		"_godot_rtc_channels", channels_cb
	)
	JavaScriptBridge.get_interface("window").set("_godot_rtc_poll", poll_cb)
	logger.log_info("JS RTC test callbacks registered!")


var _js_callbacks: Array = []  # prevent GC


func _js_test_ping(_args) -> void:
	test_send_ping()


func _js_test_channels(_args) -> void:
	test_channel_states()


func _js_test_poll(_args) -> void:
	test_poll_packets()


## Send a test ping packet to verify WebRTC data channel works
func test_send_ping() -> void:
	if not rtc_session or not rtc_session.rtc_peer:
		logger.log_error("TEST: No RTC session!")
		return
	...
	if status != MultiplayerPeer.CONNECTION_CONNECTED:
		logger.log_error("TEST: Not connected! Status=" + str(status))
		return
	...
	var test_data = {
		...
		"from": "host" if is_host else "client",
		"my_peer_id": peer.get_unique_id()
	}
	var packet = var_to_bytes(test_data)
	...
	if is_host:
		...
		for pid in peer_client_table.keys():
			if pid != 1:
				logger.log_warning(
					(
						"TEST: HOST sending ping to peer "
						+ str(pid)
						+ ", size="
						+ str(packet.size())
					)
				...
				logger.log_warning("TEST: put_packet result=" + str(err))
	else:
		# Client sends to host
		logger.log_warning(
			"TEST: CLIENT sending ping to host, size=" + str(packet.size())
		)
...
## Check data channel states directly
func test_channel_states() -> void:
	if not rtc_session or not rtc_session.rtc_peer:
		logger.log_error("TEST: No RTC session!")
		return

	var peer = rtc_session.rtc_peer
	logger.log_warning("=== CHANNEL STATE DEBUG ===")
	logger.log_warning(
		(
			"is_host="
			+ str(is_host)
			+ ", my_unique_id="
			+ str(peer.get_unique_id())
		)
	)
	logger.log_warning("connection_status=" + str(peer.get_connection_status()))
	logger.log_warning(
		"available_packets=" + str(peer.get_available_packet_count())
	)

	# Check all peers
	for pid in peer_client_table.keys():
		if peer.has_peer(pid):
			var info = peer.get_peer(pid)
			logger.log_warning(
				(
					"Peer "
					+ str(pid)
					+ ": connected="
					+ str(info.get("connected", "?"))
				)
			)
			var channels = info.get("channels", [])
			for i in range(channels.size()):
				var ch = channels[i]
				if ch:
					logger.log_warning(
						(
							...
							+ str(ch.get_ready_state())
							+ ", buffered="
							+ str(ch.get_buffered_amount())
						)
					)


## Force poll and check for packets
func test_poll_packets() -> void:
	if not rtc_session or not rtc_session.rtc_peer:
		logger.log_error("TEST: No RTC session!")
		return

	var peer = rtc_session.rtc_peer
	logger.log_warning("=== FORCE POLL TEST ===")
	logger.log_warning(
		"Before poll: available=" + str(peer.get_available_packet_count())
	)
	peer.poll()
	logger.log_warning(
		"After poll: available=" + str(peer.get_available_packet_count())
	)
	...
	while peer.get_available_packet_count() > 0:
		...
		if data is Dictionary and data.has("_test_ping"):
			logger.log_warning(
				(
					"TEST PING RECEIVED! from="
					+ str(data.get("from"))
					+ ", timestamp="
