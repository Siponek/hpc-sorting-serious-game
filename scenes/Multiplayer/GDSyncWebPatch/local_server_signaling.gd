...
var signaling_server_url: String = "https://localhost:3000"
...
func _ready() -> void:
	...
	if OS.has_feature("web"):
		...
		if window:
			...
			signaling_server_url = "https://" + str(hostname) + ":3000"
	else:
		var local_ips = IP.get_local_addresses()
		var best_ip = "127.0.0.1"
		for ip in local_ips:
			if (ip.begins_with("192.168.") or ip.begins_with("10.") or ip.begins_with("172.")) and not ":" in ip:
				best_ip = ip
				break
		signaling_server_url = "https://" + best_ip + ":3000"
	...
	logger.log_info("LocalServerSignaling initialized (HTTP mode). URL: " + signaling_server_url)
