...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	if (
		...
		and incoming_card_data == dragged_card_from_container_node
	):
	...
	if (
		...
		final_insert_index = clamp(
			final_insert_index, 0, num_actual_cards_at_drop
		)
		...
		if target_card_at_destination != null: # Swapping with an existing card
			logger.log_info(
				(
					...
					% [
						card_to_place.value,
						target_card_at_destination.value,
						final_insert_index
					]
				)
			)
	...
	emit_signal(
		card_dropped_card_container.get_name(),
		card_to_place,
		was_in_buffer,
		original_slot_ref
	)
...
func _process(_delta: float) -> void:
	...
	if (
		container_global_rect.has_point(global_mouse_pos)
		and DragState.currently_dragged_card != null
	):
		is_dragging_card_over_self = true

		var mouse_pos_relative_to_scroll_container_viewport: Vector2 = get_local_mouse_position()
		var local_mouse_pos_x_in_card_container: float = (
			mouse_pos_relative_to_scroll_container_viewport.x
			+ scroll_horizontal
		)

		var card_spacing: float = card_container.get_theme_constant(
			"separation", "HBoxContainer"
		)
		...
		if effective_card_width > 0:
			...
			potential_insert_index = int(
				(
					(
						local_mouse_pos_x_in_card_container
						+ effective_card_width / 2.0
					)
					/ effective_card_width
				)
			)

		potential_insert_index = clamp(
			potential_insert_index, 0, num_actual_cards
		)
		...
		if (
			current_drop_placeholder.is_inside_tree()
			and current_drop_placeholder.get_parent() == card_container
		):
			current_placeholder_idx = current_drop_placeholder.get_index()

		if current_placeholder_idx != potential_insert_index:
			card_container.move_child(
				current_drop_placeholder, potential_insert_index
			)
