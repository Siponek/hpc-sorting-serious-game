...
\begin{itemize}
    \item \textbf{Single Player}: Practice sorting alone (sequential execution)
    \item \textbf{Multiplayer}: Join or host a game with others (OpenMP shared-memory simulation)
...
\paragraph{Shared Memory Access:}
\begin{itemize}
    \item All cards are visible in the shared container (simulating shared memory)
    \item Each player has private buffers for local work (thread-local storage)
    \item Players can access the shared container simultaneously
    \item Other players' private buffers are not directly visible (simulating thread-private data)
\end{itemize}
...
\begin{itemize}
    ...
    \item Ask students to identify real-world parallels (threads, shared memory)
    \item Connect game mechanics to OpenMP programming model and sequential execution
