...
\begin{itemize}
    \item Digital transformation of Professor D'Agostino's successful physical classroom experiments into a scalable, accessible mobile application
    \item Systematic mapping of HPC concepts (OpenMP shared memory, MPI distributed memory) to concrete, manipulable game mechanics
...
\begin{itemize}
    ...
    \item Private buffers representing thread-local or distributed memory
    \item Card exchanges representing message passing (MPI)
    \item Network latency representing communication overhead
...
Simulate hybrid MPI+OpenMP programming:

\begin{itemize}
    \item Multiple "nodes" (groups of players)
    \item Shared memory within nodes, message passing between nodes
    \item Hierarchical parallelism concepts
