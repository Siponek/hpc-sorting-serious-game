...
\paragraph{ER2: MPI Simulation}
\begin{itemize}
    \item The game must simulate distributed-memory parallelism
    \item Each player must receive a distinct subset of cards (data distribution)
    \item Players must not see cards in other players' private buffers (separate memory spaces)
    \item Card exchanges must represent explicit message passing
\end{itemize}
...
\begin{itemize}
    \item Provide a single-player game mode simulating OpenMP
    \item Display all cards in a scrollable container
    \item Provide 2--3 private buffer zones per player
...
\begin{itemize}
    ...
    \item Distribute cards among players (MPI-style data distribution)
    \item Synchronize card movements across all clients
    \item Hide cards in other players' private buffers
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{3.5cm}p{4cm}p{5cm}@{}}
        ...
        \midrule
        Thread (OpenMP)        & Student with access to shared desk & Player in single-player mode       \\
        Shared Memory          & Desk visible to all students       & Main card container                \\
        Thread-Local Storage   & Student's private notebook         & Player's buffer zones              \\
        Parallel Execution     & Students work simultaneously       & No turn-taking, concurrent actions \\
        Process (MPI)          & Student at separate desk           & Player in multiplayer mode         \\
        Distributed Memory     & Separate desks with own cards      & Each player's private cards        \\
        Message Passing        & Physically handing card to another & Moving card to another player      \\
        Communication Overhead & Time to walk between desks         & Network latency                    \\
        Speedup                & Multiple students finish faster    & Timer shows completion time        \\
        Load Balancing         & Even distribution of work          & Equal card distribution            \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Green}: Correctly sorted cards, success states
    \item \textbf{Yellow}: Highlighted cards during drag
    \item \textbf{Red}: Invalid moves, errors
...
\label{subsec:difficulty-progression}

To support learners at different skill levels, the game offers configurable difficulty:

\paragraph{Easy Mode (Beginner):}
\begin{itemize}
    \item 10--20 cards
    \item Completely random shuffle
    \item Single-player only
    \item Visual hints (correct position indicators)
\end{itemize}

\paragraph{Medium Mode (Intermediate):}
\begin{itemize}
    \item 30--50 cards
    \item Random or partially sorted
    \item Single-player or 2-player multiplayer
    \item No visual hints
\end{itemize}

\paragraph{Hard Mode (Advanced):}
\begin{itemize}
    \item 50--200 cards
    \item Reverse sorted (worst case)
    \item 3--4 player multiplayer
    \item Time limits or move limits
\end{itemize}
