...
This chapter describes the comprehensive design and architecture of the HPC Sorting Serious Game. We present the system from multiple perspectives: high-level scene structure, core component design, multiplayer networking architecture, mobile UI considerations, and detailed data flow patterns.

%----------------------------------------------------------------------------------------
\section{Architectural Overview}
\label{sec:architectural-overview}

The game follows a component-based architecture typical of modern game engines, with clear separation of concerns between presentation, game logic, and networking layers. The architecture is designed to support both single-player (OpenMP simulation) and multiplayer (MPI simulation) modes with significant code reuse.
...
\begin{enumerate}
	...
	\item \textbf{Mobile-First UI}: All interfaces are designed primarily for touch input on small screens, with responsive layouts that adapt to different device sizes.
...
\begin{description}
	\item[Presentation Layer] Handles all visual rendering, animations, user input (touch events), and UI components (menus, dialogs, toasts).
	...
	\item[Networking Layer] Manages peer-to-peer connections via WebRTC, state synchronization through GDSync, lobby system for player matchmaking, and host election.
...
\begin{itemize}
	...
	\item Exit to system (on Android)
\end{itemize}

\paragraph{Key Components:}
\begin{lstlisting}[language=GDScript, caption={Main menu scene structure (simplified)}]
...
\begin{lstlisting}[language=GDScript, caption={Single-player scene hierarchy}]
...
\begin{itemize}
	\item \textbf{ScrollContainer for Cards}: Enables viewing large numbers of cards (50--200) on small mobile screens by providing vertical scrolling.
...
\begin{lstlisting}[language=GDScript, caption={Lobby scene structure}]
LobbyScene (Control)
├── ConnectionManager (Node) [Script: connection_manager.gd]
│   ├── WebRTCConnection (Node)
...
\begin{enumerate}
	...
	\item Signaling server facilitates WebRTC peer discovery and connection
...
Implements the MPI simulation gameplay with distributed card ownership and explicit message passing.

\paragraph{Responsibilities:}
\begin{itemize}
	\item Distribute cards across players (each receives a subset)
	\item Display only local cards and public container
	\item Hide cards currently in other players' buffers
	\item Synchronize card movements across all clients
	\item Enable card exchanges between players
...
\begin{lstlisting}[language=GDScript, caption={Multiplayer scene hierarchy}]
...
\begin{lstlisting}[language=GDScript, caption={Card component class definition}]
class_name Card
extends Control
...
\begin{lstlisting}[language=GDScript, caption={Card drag-and-drop logic}]
...
\begin{lstlisting}[language=GDScript, caption={CardManager class structure}]
...
\begin{lstlisting}[language=GDScript, caption={CardManager core methods}]
...
\begin{lstlisting}[language=GDScript, caption={MultiplayerCardManager inheritance}]
...
\begin{lstlisting}[language=GDScript, caption={Key synchronization methods}]
...
A critical aspect of the MPI simulation is maintaining correct visibility of cards:

\begin{itemize}
	\item \textbf{Cards in main container}: Visible to all players
	\item \textbf{Cards in my buffers}: Visible only to me
	\item \textbf{Cards in other players' buffers}: Hidden from view (simulating distributed memory)
	\item \textbf{Cards being transferred}: Brief transition animation
\end{itemize}

The `cards_in_other_buffers` dictionary tracks which cards are currently private:

\begin{lstlisting}[language=GDScript, caption={Visibility tracking}]
...
Managing 50--200 cards on a mobile screen requires efficient layout and scrolling.

\paragraph{Architecture:}

\begin{lstlisting}[language=GDScript, caption={Scrollable card container structure}]
CardScrollContainer (ScrollContainer)
...
\begin{itemize}
	...
	\item \textbf{Touch-Optimized}: Large touch targets, scroll physics tuned for mobile
\end{itemize}

\paragraph{Drop Target Detection:}

\begin{lstlisting}[language=GDScript, caption={Drop slot calculation}]
...
\begin{itemize}
	\item Each player has direct WebRTC connection to every other player
...
\paragraph{Pattern 2: Request-Confirm}

Used for critical actions that require validation:

\begin{enumerate}
	\item Client requests action from host (e.g., taking card from shared container)
	\item Client shows "pending" state (e.g., semi-transparent card)
	\item Host validates availability
	\item Host broadcasts confirmation or rejection
	\item Client applies final state
\end{enumerate}

\paragraph{Pattern 3: Host Broadcast}
...
GDSync is a third-party multiplayer framework for Godot that provides high-level abstractions for state synchronization.
...
\begin{lstlisting}[language=GDScript, caption={GDSync setup in MultiplayerCardManager}]
func _ready():
...
\begin{lstlisting}[language=GDScript, caption={Using GDSync RPCs}]
...
\begin{enumerate}
	...
	\item Players connect via WebRTC signaling
...
\begin{lstlisting}[language=GDScript, caption={Handling player disconnections}]
...
\begin{itemize}
	...
	\item When Player 3 returns the card to the container, `sync_card_left_buffer` makes it visible again to everyone
\end{itemize}

%----------------------------------------------------------------------------------------
\section{Mobile UI/UX Design}
\label{sec:mobile-ui-ux}

%----------------------------------------------------------------------------------------
\subsection{Design Constraints}
\label{subsec:design-constraints}

Mobile platforms impose several constraints that significantly influenced design decisions:
...
\begin{itemize}
	\item Cards sized dynamically: `min(screen_width / 6, 120px)`
...
\begin{itemize}
	...
	\item ``Connection lost, attempting reconnect...''
	\item ``Invalid move!''
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Invalid Move}: Shake horizontally 3 times (300ms)
	\item \textbf{Sorting Success}: Green highlight wave across cards (1000ms)
...
\begin{itemize}
	...
	\item \textbf{Correct Position}: Green background (when sorted correctly)
...
\begin{itemize}
	...
	\item \textbf{Target Frame Rate}: 60 FPS on mid-range devices (2020+ Android phones)
...
\begin{itemize}
	...
	\item Mobile UI/UX design strategies and constraints
	\item Performance optimization considerations
\end{itemize}

The architecture balances educational effectiveness with technical feasibility, prioritizing clarity of HPC concept representation while maintaining acceptable performance on mobile devices. The modular, component-based design allows for future extensions and modifications, fulfilling the extensibility objective outlined in Chapter~\ref{ch:introduction}.
