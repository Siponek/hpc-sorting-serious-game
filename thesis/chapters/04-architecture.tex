...
GDSync is a third-party multiplayer framework for Godot that provides high-level abstractions for state synchronization. Importantly, \textbf{GDSync internally handles all WebRTC connection management}, including peer discovery, connection establishment, and data channel management. The game primarily interacts with GDSync's RPC API rather than directly managing WebRTC connections.
...
\begin{itemize}
    ...
    \item \textbf{WebRTC Abstraction}: GDSync internally manages WebRTC connections, signaling, and peer-to-peer data channels
\end{itemize}
